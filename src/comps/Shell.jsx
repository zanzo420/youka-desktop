  ...
  return (
    <div>
      <div className="flex flex-row w-full justify-between p-2 mb-2 bg-primary">
        ...
        <Input className="p-2 px-2 w-2/4 flex-2" type="text" onChange={handleSearchChange} placeholder="Search" onFocus={handleSearchFocus} ref={searchRef} />
        <div className="flex-1"></div>
      </div>
      {children}
      <div className="flex flex-row justify-center">
        <div style={{ color: playlist === PLAYLIST_SEARCH ? "#E30B17" : "black" }} className="p-4 text-xl cursor-pointer" onClick={handleSearch}>Search</div>
        <div style={{ color: playlist === PLAYLIST_TRENDING ? "#E30B17" : "black" }} className="p-4 text-xl cursor-pointer" onClick={handleTrending}>Trending</div>
        {youtubeID ? <div style={{ color: playlist === PLAYLIST_MIX ? "#E30B17" : "black" }} className="p-4 text-xl cursor-pointer" onClick={handleMix}>Mix</div> : null}
        <div style={{ color: playlist === PLAYLIST_LIBRARY ? "#E30B17" : "black" }} className="p-4 text-xl cursor-pointer" onClick={handleLibrary}>Library</div>
      </div>
      {
        loading ?
